...
static ListHead free_list, devices;
...
void init_hal(void) {
	...
	list_init(&free_list);
	for (i = 0; i < NR_DEV; i ++) {
		list_add_before(&free_list, &dev_pool[i].list);
	}
	list_init(&devices);
}
...
void hal_register(const char *name, pid_t pid, int dev_id) {
	...
	Device *dev = list_entry(free_list.next, Device, list);
	list_del(&dev->list);
	dev->name = name;
	dev->pid = pid;
	dev->dev_id = dev_id;
